...
class nsc_bar_cookie_handler
{
    ...
    public function nsc_bar_set_itp_cookie()
    {
        if ($this->plugin_configs->nsc_bar_get_option('activate_banner') == true &&
            ...
            if (isset($_COOKIE[$this->cookie_configs['name']])) {
                setcookie($this->cookie_configs['name'], $_COOKIE[$this->cookie_configs['name']], $expiryDate, $this->cookie_configs['path'], $this->cookie_configs['domain'], $this->cookie_configs['secure']);
                setcookie(ITP_SAVER_COOKIE_NAME, $this->cookie_configs['name'] . "---_---" . $expiryDate, $expiryDate, $this->cookie_configs['path'], $this->cookie_configs['domain'], $this->cookie_configs['secure'], true);
            }
            ...
            if (!empty($this->cookie_configs['cookietypes'])) {
                foreach ($this->cookie_configs['cookietypes'] as $cookietype_configs) {
                    ...
                    if (isset($_COOKIE[$differentiatedCookieName])) {
                        setcookie($differentiatedCookieName, $_COOKIE[$differentiatedCookieName], $expiryDate, $this->cookie_configs['path'], $this->cookie_configs['domain'], $this->cookie_configs['secure']);
                    }
                }
            }
        }
    ...
    public function nsc_bar_set_default_cookies()
    {
        ...
        if (in_array($this->cookie_configs['compliance_type'], array("detailed", "detailedRev")) && !empty($this->cookie_configs['cookietypes'])) {
            foreach ($this->cookie_configs['cookietypes'] as $cookietype_configs) {
                ...
                if (!isset($_COOKIE[$differentiatedCookieName])) {
                    setcookie($differentiatedCookieName, $value, $expiryDate, $this->cookie_configs['path'], $this->cookie_configs['domain'], $this->cookie_configs['secure']);
                }
            }
        }
    }
    ...
    public function nsc_bar_migrate_cookie_detailed_to_savesettings()
    {
        if (isset($_COOKIE[$this->cookie_configs['name']]) && $_COOKIE[$this->cookie_configs['name']] == "detailed") {
            $expiryDate = $this->get_expiry_date($this->cookie_configs['name'], $this->cookie_configs['expirydays']);
            setcookie($this->cookie_configs['name'], "savesettings", $expiryDate, $this->cookie_configs['path'], $this->cookie_configs['domain'], $this->cookie_configs['secure']);
        }
    }
    ...
    public function nsc_bar_cookie_cleanup()
    {
        ...
        $current_cookie_value = $_COOKIE[$this->cookie_configs['name']];
    ...
    public function nsc_bar_delete_cookie($delete_detailed = true)
    {
        if (isset($_COOKIE[$this->cookie_configs['name']])) {
            ...
            setcookie($this->cookie_configs['name'], "emptyvalue", time() - 3600, $this->cookie_configs['path'], $this->cookie_configs['domain']);
            ...
            if (isset($_COOKIE[ITP_SAVER_COOKIE_NAME])) {
                ...
                setcookie(ITP_SAVER_COOKIE_NAME, "emptyvalue", time() - 3600, $this->cookie_configs['path'], $this->cookie_configs['domain']);
            }
        ...
        if (!empty($this->cookie_configs['cookietypes'])) {
            foreach ($this->cookie_configs['cookietypes'] as $cookietype_configs) {
                ...
                if (isset($_COOKIE[$differentiatedCookieName])) {
                    ...
                    setcookie($differentiatedCookieName, "emptyvalue", time() - 3600, $this->cookie_configs['path'], $this->cookie_configs['domain']);
                }
            }
        }
    }
